...
import json
from .constants_tests import TEST_AC_NAME
...
class ViewTest(TestCase):
    def setUp(self):
        ...
        self.case1 = AssuranceCase.objects.create(
            name=TEST_AC_NAME+"1", description="test description1", shape=0
        )
        self.case2 = AssuranceCase.objects.create(
            name=TEST_AC_NAME+"2", description="test description2", shape=0
        )
        self.case_delete = AssuranceCase.objects.create(
            name=TEST_AC_NAME+"del", description="to delete description2",
            shape=0
        )
        self.valid_entry = {
            "name": "TestAC_updated",
