...
const SMALL_TITLE_REGEX = /small title/i;
const CONTENT_SECTION_1_REGEX = /content section 1/i;
const CANCEL_REGEX = /cancel/i;
const PARENT_CARD_REGEX = /parent card/i;
const NESTED_CARD_REGEX = /nested card/i;
const NESTED_CONTENT_REGEX = /nested content/i;
const ACCESSIBLE_CARD_REGEX = /accessible card/i;
...
describe("Card", () => {
	...
	describe("Card composition", () => {
		...
		it("should maintain proper spacing with custom classes", () => {
			...
			const card = screen.getByText(SMALL_TITLE_REGEX).closest(".rounded-lg");
			expect(card).toHaveClass("w-full", "max-w-md");

			const header = screen.getByText(SMALL_TITLE_REGEX).parentElement;
			expect(header).toHaveClass("pb-4");

			const content = screen.getByText(CONTENT_SECTION_1_REGEX).parentElement;
			expect(content).toHaveClass("space-y-4");

			const footer = screen.getByText(CANCEL_REGEX).parentElement;
		...
		it("should support nested cards", () => {
			...
			expect(screen.getByText(PARENT_CARD_REGEX)).toBeInTheDocument();
			expect(screen.getByText(NESTED_CARD_REGEX)).toBeInTheDocument();
			expect(screen.getByText(NESTED_CONTENT_REGEX)).toBeInTheDocument();

			// Both cards should have the card styles
			const cards = screen
				.getByText(PARENT_CARD_REGEX)
				.closest(".rounded-lg")
		...
		it("should support accessibility attributes on all components", () => {
			renderWithoutProviders(
				<Card aria-label="Main card">
					...
					<CardFooter>Footer with role</CardFooter>
				</Card>
			);

			const card = screen.getByLabelText("Main card");
			expect(card).toBeInTheDocument();

			const title = screen.getByText(ACCESSIBLE_CARD_REGEX);
			expect(title).toHaveAttribute("id", "card-title");

			const footer = screen.getByText("Footer with role");
			expect(footer).toBeInTheDocument();
		});
	});
});
