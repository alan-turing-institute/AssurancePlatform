 ...
 * Each handler receives { node, edge, nodes, edges, setNodes, setEdges, reactFlowInstance, position, ...context }
 */
export const menuActions = {
  ...
  markComplete: ({ node, nodes, setNodes }) => {
    setNodes(nodes.map(n =>
      n.id === node.id
        ? { ...n, data: { ...n.data, completed: true, completedAt: new Date().toISOString() } }
        : n
    ));
  ...
  verify: ({ node, nodes, setNodes }) => {
    setNodes(nodes.map(n =>
      n.id === node.id
        ? { ...n, data: { ...n.data, verified: true, verifiedAt: new Date().toISOString() } }
        : n
    ));
  ...
  updateStatus: ({ node, nodes, setNodes, status }) => {
    setNodes(nodes.map(n =>
      n.id === node.id
        ? { ...n, data: { ...n.data, status, statusUpdatedAt: new Date().toISOString() } }
        : n
    ));
  ...
  align: ({ selectedNodes, nodes, setNodes, alignment }) => {
    ...
    switch (alignment) {
      case 'left':
        ...
        break;
      case 'right':
        alignValue = Math.max(...positions.map((p, i) => p.x + (selectedNodes[i].width || 200)));
        setNodes(nodes.map(n => {
          const selected = selectedNodes.find(sn => sn.id === n.id);
          return selected
            ? { ...n, position: { ...n.position, x: alignValue - (n.width || 200) } }
            : n;
        }));
        break;
      case 'center':
        const minX = Math.min(...positions.map(p => p.x));
        const maxX = Math.max(...positions.map((p, i) => p.x + (selectedNodes[i].width || 200)));
        alignValue = (minX + maxX) / 2;
        setNodes(nodes.map(n => {
          const selected = selectedNodes.find(sn => sn.id === n.id);
          return selected
            ? { ...n, position: { ...n.position, x: alignValue - (n.width || 200) / 2 } }
            : n;
        }));
        break;
      case 'top':
        ...
        break;
      case 'bottom':
        alignValue = Math.max(...positions.map((p, i) => p.y + (selectedNodes[i].height || 100)));
        setNodes(nodes.map(n => {
          const selected = selectedNodes.find(sn => sn.id === n.id);
          return selected
            ? { ...n, position: { ...n.position, y: alignValue - (n.height || 100) } }
            : n;
        }));
        break;
      case 'middle':
        const minY = Math.min(...positions.map(p => p.y));
        const maxY = Math.max(...positions.map((p, i) => p.y + (selectedNodes[i].height || 100)));
        alignValue = (minY + maxY) / 2;
        setNodes(nodes.map(n => {
          const selected = selectedNodes.find(sn => sn.id === n.id);
          return selected
            ? { ...n, position: { ...n.position, y: alignValue - (n.height || 100) / 2 } }
            : n;
        }));
        break;
    }
  ...
  group: ({ selectedNodes, onGroup }) => {
    ...
    console.log('Grouped nodes:', selectedNodes.map(n => n.id));
  },
  ...
  changeEdgeStyle: ({ edge, edges, setEdges, edgeStyle }) => {
    ...
    setEdges(edges.map(e =>
      e.id === edge.id
        ? {
            ...e,
            style: { ...e.style, ...styleMap[edgeStyle] },
            animated: styleMap[edgeStyle].animated
          }
        : e
    ));
  ...
  changeStrength: ({ edge, edges, setEdges, strength }) => {
    ...
    setEdges(edges.map(e =>
      e.id === edge.id
        ? { ...e, style: { ...e.style, strokeWidth: widthMap[strength] } }
        : e
    ));
  ...
  paste: ({ clipboard, position, nodes, setNodes }) => {
    ...
    if (clipboard.type === 'nodes') {
      const newNodes = clipboard.data.map((node, index) => ({
        ...node,
   ...
   * Auto layout
   */
  autoLayout: ({ layoutType, nodes, edges, setNodes, onAutoLayout }) => {
    if (onAutoLayout) {
 ...
 * Execute menu action
 *
 * @param {string} actionString - Action string (e.g., 'delete', 'setPriority:high')
 * @param {Object} context - Context object with all necessary data
 */
export function executeAction(actionString, context) {
  ...
  const enhancedContext = param
    ? { ...context, [action]: param, priority: param, strategyType: param, status: param, confidence: param, importance: param, alignment: param, direction: param, edgeType: param, edgeStyle: param, strength: param, nodeType: param, layoutType: param, format: param }
    : context;
 ...
 */
export function checkClipboard(context) {
  return !!(context.clipboard && context.clipboard.data);
}
