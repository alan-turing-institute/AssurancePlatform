...
async function fetchCaseFromDjango(
	caseId: string,
	token: string
): Promise<CaseResult> {
	const response = await fetch(
		`${process.env.API_URL || process.env.NEXT_PUBLIC_API_URL}/api/cases/${caseId}/`,
		{
			headers: {
...
export async function GET(
	...
	const result: CaseResult = USE_PRISMA_AUTH
		? await fetchCaseFromPrisma(id, session.key)
		: await fetchCaseFromDjango(id, session.key);
...
/**
 * Handles Django-based case update
 */
async function updateCaseWithDjango(
	id: string,
	sessionKey: string,
	body: Record<string, unknown>
): Promise<NextResponse> {
	const response = await fetch(
		`${process.env.API_URL || process.env.NEXT_PUBLIC_API_URL}/api/cases/${id}/`,
		{
			method: "PUT",
			headers: {
	...
	if (!response.ok) {
		...
		return NextResponse.json(
			{ error: error || "Failed to update case" },
...
export async function PUT(
	...
	try {
		...
		return await updateCaseWithDjango(id, session.key, body);
	} catch (error) {
...
export async function DELETE(
	...
	if (USE_PRISMA_AUTH) {
		...
		if (
			!permissionResult.hasAccess ||
			permissionResult.permission !== "ADMIN"
		) {
			return NextResponse.json({ error: "Permission denied" }, { status: 403 });
		}
	...
	try {
		...
		if (!response.ok) {
			const error = await response.text();
			return NextResponse.json(
				{ error: error || "Failed to delete case" },
				{ status: response.status }
			);
		}
		...
		console.error("Error proxying case delete to Django:", error);
		return NextResponse.json(
			{ error: "Failed to delete case" },
			{ status: 500 }
		);
