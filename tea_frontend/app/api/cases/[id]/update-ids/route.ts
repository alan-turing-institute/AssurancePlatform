...
/**
 * Resets element identifiers via Django API
 */
async function resetIdentifiersWithDjango(
	caseId: string,
	token: string
): Promise<{ success: boolean; error?: string; status: number }> {
	const djangoUrl = `${process.env.NEXT_PUBLIC_API_URL ?? process.env.NEXT_PUBLIC_API_URL_STAGING}/api/cases/${caseId}/update-ids`;
	...
	if (!response.ok) {
		...
		return {
			success: false,
			error: error || "Failed to reset identifiers",
...
export async function POST(
	...
	try {
		...
		if (USE_PRISMA_AUTH) {
			...
			result = await resetIdentifiersWithPrisma(caseId, validation.userId);
		} else {
			result = await resetIdentifiersWithDjango(caseId, session.key);
		}
