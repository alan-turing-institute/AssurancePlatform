...
vi.mock('../page', () => ({
  default: () => {
    const { DiscoverPageMock } = require('./mocks/DiscoverPageMock');
...
describe('Published Case Flow Integration Tests', () => {
  ...
  describe('Viewing Published Cases', () => {
    it('should display list of published cases on discover page', async () => {
      ...
      const { DiscoverPageMock } = await import('./mocks/DiscoverPageMock');
  ...
  describe('Search and Filter Public Cases', () => {
    ...
    it('should filter public cases by tags', async () => {
      ...
      const safetyOption = screen.getByRole('option', {
        name: REGEX_PATTERNS.safety,
      });
      ...
      server.use(
        http.get(
          ...
          ({ request }) => {
            ...
            const tags = url.searchParams.get('tags');
            if (tags === 'safety') {
              return HttpResponse.json([taggedCases[0], taggedCases[1]]);
            }
            return HttpResponse.json(taggedCases);
          }
        )
      ...
      const applyButton = screen.getByRole('button', {
        name: REGEX_PATTERNS.applyFilters,
      });
