 ...
 *
 * Algorithm notes (for efficiency):
 * - Uses a single DB query filtered by parentId to count siblings
 * - For property claims under another property claim: hierarchical naming (P1.1, P1.1.1)
 * - For top-level property claims (under strategy/goal): sequential (P1, P2)
 * - For other elements: sequential by type within case (G1, S1, E1, C1)
 *
 * Time complexity: O(1) DB query (indexed on parentId/caseId)
...
async function generateElementName(
	...
	// Other element types with a parent - count siblings under the same parent
	if (parentId) {
		const siblingCount = await prismaNew.assuranceElement.count({
			where: {
				parentId,
	...
	// Root-level elements (no parent) - count all of same type in case
	const caseCount = await prismaNew.assuranceElement.count({
		where: {
			caseId,
			elementType: elementType as PrismaElementType,
			parentId: null,
