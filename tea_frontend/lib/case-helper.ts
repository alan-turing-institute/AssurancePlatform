...
const searchAndUpdateInStrategiesClaims = (
	...
	for (const strategy of strategies) {
		if (strategy.property_claims && strategy.property_claims.length > 0) {
			...
			// Check if the array was actually modified (by comparing references)
			if (updatedNestedArray !== strategy.property_claims) {
...
const processPropertyClaimForUpdate = (
	...
	if (propertyClaim.property_claims?.length) {
		...
		// Check if the nested array was actually modified
		if (result !== propertyClaim.property_claims) {
			return array;
		}
	}
...
export const updatePropertyClaimNested = (
	...
	return array; // Return unchanged array if item not found
};
