...
async function testKeyboardInteractions(
  ...
  element.addEventListener('keydown', (e) => {
    if (e.key === 'Enter') enterTriggered = true;
  });
  ...
  element.addEventListener('keydown', (e) => {
    if (e.key === 'Escape') escapeTriggered = true;
  });
...
function testLandmarkNavigation(container: HTMLElement): {
  ...
  if (!hasMainLandmark) missingLandmarks.push('main');
  if (!hasNavigationLandmark) missingLandmarks.push('navigation');
...
async function testFocusTrap(
  ...
  if (focusableElements.length < 2) return true; // Can't test trap with fewer than 2 elements
...
function checkLogicalTabOrder(elements: HTMLElement[]): boolean {
  ...
  for (let i = 1; i < elements.length; i++) {
    ...
    if (Math.abs(current.top - previous.top) < 10) {
      if (current.left < previous.left - 10) {
        return false;
      }
    }
  }
