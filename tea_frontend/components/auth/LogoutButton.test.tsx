...
import { render, screen, renderWithAuth, renderWithoutProviders } from '@/src/__tests__/utils/test-utils'
...
describe('LogoutButton', () => {
  ...
  it('should render logout button with icon and accessibility label', () => {
    ...
    expect(screen.getByText('Logout', { selector: '.sr-only' })).toBeInTheDocument()
  ...
  it('should handle successful logout', async () => {
    ...
    server.use(
      http.post(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000'}/api/auth/logout/`, () => {
  ...
  it('should send correct authorization header in logout request', async () => {
    ...
    server.use(
      http.post(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000'}/api/auth/logout/`, ({ request }) => {
