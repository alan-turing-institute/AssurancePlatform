...
import { render, screen, renderWithAuth } from '@/src/__tests__/utils/test-utils'
...
describe('CaseCard', () => {
  ...
  it('should render case information correctly', () => {
    ...
    expect(screen.getByText('A comprehensive safety assurance case')).toBeInTheDocument()
  ...
  it('should open delete confirmation modal when delete button is clicked', async () => {
    ...
    expect(screen.getByText('Are you sure you want to delete this case?')).toBeInTheDocument()
  })
  ...
  it('should handle successful case deletion', async () => {
    ...
    server.use(
      http.delete(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000'}/api/cases/1/`, () => {
  ...
  it('should handle long case names and descriptions', () => {
    const caseWithLongText = createMockAssuranceCase({
      ...
      description: 'This is a very long description that contains a lot of text to test how the component handles text overflow and maintains proper layout structure',
    })

    renderWithAuth(<CaseCard assuranceCase={caseWithLongText} />)

    expect(screen.getByText(/This is a very long case name/)).toBeInTheDocument()
