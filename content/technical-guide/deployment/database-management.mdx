...
```shell
# Create a backup
docker exec tea_postgres pg_dump -U tea tea > backup-$(date +%Y%m%d-%H%M%S).sql

# Compressed backup
docker exec tea_postgres pg_dump -U tea tea | gzip > backup-$(date +%Y%m%d-%H%M%S).sql.gz
```
...
0 2 * * * docker exec tea_postgres pg_dump -U tea tea | gzip > /backups/tea-$(date +\%Y\%m\%d).sql.gz
```
...
docker-compose -f docker-compose.yml stop tea_app

# Restore from backup
cat backup.sql | docker exec -i tea_postgres psql -U tea tea

# Or from compressed backup
gunzip -c backup.sql.gz | docker exec -i tea_postgres psql -U tea tea

# Start the application
docker-compose -f docker-compose.yml start tea_app
```
...
```shell
# 1. Backup first!
docker exec tea_postgres pg_dump -U tea tea > pre-reset-backup.sql

# 2. Stop application
docker-compose -f docker-compose.yml stop tea_app

# 3. Connect to database
docker exec -it tea_postgres psql -U tea tea
...
# Docker
docker exec -it tea_postgres_dev psql -U tea tea
```
