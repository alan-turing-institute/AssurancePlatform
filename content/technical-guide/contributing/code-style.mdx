...
import type { AssuranceCase, AssuranceElement } from '@prisma/client'
```
...
async function CasePage({ params }: { params: Promise<{ id: string }> }) {
...
export async function createElement(
  ...
  const result = createElementSchema.safeParse(input)
  if (!result.success) return { success: false, error: 'Invalid input' }

  // Dynamic import for Prisma (keeps Server Action bundle small)
  const { prisma } = await import('@/lib/prisma')

  // Check permissions in service layer
  const hasAccess = await checkCaseAccess(result.data.caseId, session.userId, 'EDIT')
  if (!hasAccess) {
...
Actions should return consistent result objects (never throw for expected failures):
