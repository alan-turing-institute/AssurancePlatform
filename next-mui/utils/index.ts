 ...
 */
function sanitizeForHtml(input_text: any, replaceNewLines = false) {
 ...
 */
function decodeFromHtml(input_text : any) {
  return input_text
    .replace(/&amp;/g, "&")
    .replace(/&lt;/g, "<")
    .replace(/&gt;/g, ">")
 ...
 * @returns {void}
 */
function removeArrayElement(array: any, element: any) {
...
function jsonToMermaid(
   ...
   */
  function makeBox(item: any, shape: any, name: any, isCollapsed: any) {
 ...
 */
function splitCommaSeparatedString(string: any) {
 ...
 */
function joinCommaSeparatedString(array: any) {
  return array.join();
}
 ...
 */
function visitCaseItem(caseItem: any, callback: any, itemType = "TopLevelNormativeGoal") {
  ...
  configData.navigation[itemType]["children"].forEach((childName: any, j: any) => {
    let childType = configData.navigation[itemType]["children"][j];
 ...
 */
function getParentPropertyClaims(assuranceCase: any, id: any, type: any) {
  ...
  while (caseItemStack.length > 0) {
    ...
    configData.navigation[nodeType]["children"].forEach((childName: any, j: any) => {
