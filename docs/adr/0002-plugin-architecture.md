...
- Prisma back-relations declared on both ends; no cascade from `Integration` to `User` — at the database level, deleting a human owner (or a machine principal) is blocked **unconditionally while any integration row references them** (`ON DELETE RESTRICT`); the registry service layers the humane flow on top (reassign or delete registrations first). Deliberately stricter than blocking only on *active* registrations: a revoked integration keeps its accountability trail, so its owner reference must not silently vanish. *(Wording tightened 2026-07-03 after implementation review confirmed the RESTRICT behaviour as built.)*
...
- **Snapshots:** `PublishedAssuranceCase` content gains a `pluginData` section capturing every plugin namespace **holding data on the case** (capture follows data present, not any viewer's toggle) — a snapshot remains a citable point-in-time record, plugins included, without core understanding any of it. *(Erratum, 2026-07-13: this item originally named `ReleaseSnapshot`, a schema model unused by application code; `PublishedAssuranceCase` is the model that actually stores per-case publish snapshots and is what work item 6 implemented.)*
