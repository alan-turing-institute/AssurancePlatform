...
<Tabs>
<TabItem value="sqllite" label="SQLite">

```python
# Used by default if no environment variables are set.
...
```python
# For production, environment variables such as DBHOST, DBNAME, DBUSER, and DBPASSWORD need to be defined.
# This setup enhances the platform's scalability and security in production deployments.
